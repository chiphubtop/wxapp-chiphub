  ...
  fetchData: function (id) {
    ...
    wx.request({
      ...
      success: function (res) {
        if(!res.data.error) {
          res.data.topic.body = res.data.topic.body.replace(/<[^>]+>/g, '').replace(/\n{3,}/g, '\n\n').replace(/\(\/uploads/g, '(https://chiphub.top/uploads');
          res.data.topic.created_at = util.getDateDiff(new Date(res.data.topic.created_at));
          if (res.data.topic.user.avatar_url.indexOf('chiphub') === -1) {
            res.data.topic.user.avatar_url = 'https://chiphub.top/' + res.data.topic.user.avatar_url;
          }
      ...
      success: function (res) {
        if(res.data.replies.length > 0) {
          var mReplies = self.data.replies.concat(res.data.replies.map(item => {
            ...
            if (item.user.avatar_url.indexOf('chiphub') === -1) {
            ...
            item.body_html = item.body_html.replace(/src=\"\//g, 'src=\"https://chiphub.top/');
